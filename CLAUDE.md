...
- **Cloudflare Worker** proxies Anthropic API calls (`worker/` — not yet scaffolded; lands in Week 4. Until then any AI calls return mock JSON.)
...
**Rules:**
- Never hardcode hex values in components. Always use Tailwind utility classes (`bg-bg-surface`, `text-text-secondary`, `border-border`).
- Two layers of tokens: **ramps** (`mulberry-50` through `mulberry-950`, same for `sanmarino-*`) and **semantic aliases** (`bg-app`, `text-headline`, `action`, `action-hover`, etc.). Reach for semantic aliases first; only use raw ramp shades when no alias fits.
...
- **Domine** for display/headlines and the wordmark (weights 400, 500, 600, 700 loaded)
- **Space Grotesk** for body and UI (weights 300, 400, 500, 600, 700 loaded)
- **JetBrains Mono** for labels, timestamps, counts (weights 400, 500, 700 loaded)

Loaded from Google Fonts via `<link>` tags in `index.html` with `display=swap`.

A full type scale (sizes, line heights, weights per role) is **deferred** until the matching variables are set up in Figma — we mirror Figma into code, not the other way around, to avoid drift.
...
- Domain swap from `*.pages.dev` to `reckonapp.com`
...
Production deploy is automatic: any push to `main` triggers a Cloudflare Pages build. Live preview: https://reckon-a5q.pages.dev (the `*.pages.dev` subdomain stays through v0.1; `reckonapp.com` is a post-launch domain swap).
